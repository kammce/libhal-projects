...
main()
{
  ...
  (void)hal::write(terminal, "Starting RMD Demo\n");
  ...
  auto drc = hal::rmd::drc::create(router, 6.0f, 0x141).value();

  while (true) {
    using namespace std::chrono_literals;
    drc.position_control(45.0_deg, 30.0_rpm);
    print_feedback_and_delay(drc, terminal, steady_clock, 2s).value();
    drc.position_control(-45.0_deg, 30.0_rpm);
    print_feedback_and_delay(drc, terminal, steady_clock, 2s).value();
    drc.position_control(0.0_deg, 30.0_rpm);
    print_feedback_and_delay(drc, terminal, steady_clock, 2s).value();
    drc.velocity_control(30.0_rpm);
    print_feedback_and_delay(drc, terminal, steady_clock, 4s).value();
    drc.velocity_control(-30.0_rpm);
    print_feedback_and_delay(drc, terminal, steady_clock, 1s).value();
    print_feedback_and_delay(drc, terminal, steady_clock, 1s).value();
    print_feedback_and_delay(drc, terminal, steady_clock, 1s).value();
    print_feedback_and_delay(drc, terminal, steady_clock, 1s).value();
