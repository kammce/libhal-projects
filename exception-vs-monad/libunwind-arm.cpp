...
constexpr int phase1_vrs_size = sizeof(phase1_vrs);  // 488 bytes
constexpr int wmmxd_size = sizeof(wmmxd_regs);       // 128 bytes
constexpr int wmmxc_size = sizeof(wmmxc_regs);       // 16 bytes
constexpr int vfp_regs_size = sizeof(vfp_regs);      // 136 bytes
constexpr int vfpv3_regs_size = sizeof(vfpv3_regs);  // 128 bytes
...
constexpr int phase2_vrs_size = sizeof(phase2_vrs);  // 72 bytes
constexpr int core_regs_size = sizeof(core_regs);    // 64 bytes
...
extern _Unwind_Reason_Code __aeabi_unwind_cpp_pr0(
  ...
  _Unwind_Context*);  // __attribute__((weak));
  ...
  _Unwind_VRS_DataRepresentation representation)
{
  ...
  switch (regclass) {
    ...
    case _UVRSC_VFP: {
      ...
      if (num_vfpv3_regs > 0 && (vrs->demand_save_flags & DEMAND_SAVE_VFP_V3)) {
...
inline _Unwind_Ptr _Unwind_GetIPInfo(struct _Unwind_Context* context,
                                     int* ip_before_insn)
{
  *ip_before_insn = 0;
